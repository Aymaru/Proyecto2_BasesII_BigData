...
CREATE PROCEDURE CDBSP_INSERT_CAMPAIGN 
 as
	...
	declare @idsCitiesxCountry as table (idCityXCountry int)
	...
	-- Genera una duracion aleatoria entre 4 y 9 dias
	select @random_duration = cast (RAND () * 6 as int) + 4
	...
	select @cost = cast (RAND () * 999999 as int)
	...
	-- Genera un costo aleatorio
	select @cost = cast (RAND () * 999999 as int)
	select @publicAmount = cast (RAND () * 99999 as int)
	...
	-- Las campañas se realizan entre 1 y 5 ciudades
	delete @idsCitiesxCountry
	select @tmp = cast (RAND() * 5 as int) + 1
	...
	while (@tmp > 0)
		begin
			...
			if not exists (select * from @idsCitiesxCountry where @idCityxCountry = idCityXCountry ) 
				begin
					-- Si no se ha agregado aun, lo agrega a la tabla temporal para validar repetidos
					insert into @idsCitiesxCountry values (@idCityxCountry)
